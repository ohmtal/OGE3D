...
sha1_finish_ctx (struct sha1_ctx *ctx, void *resbuf)
{
  ...
  ctx->buffer[size - 1] = SWAP (ctx->total[0] << 3);
   ...
   dMemcpy (&((char *) ctx->buffer)[bytes], fillbuf, (size - 2) * 4 - bytes);
  ...
  if (ctx->buflen != 0)
    {
      ...
      size_t add = 128 - left_over > len ? len : 128 - left_over;
       ...
       dMemcpy (&((char *) ctx->buffer)[left_over], buffer, add);
      ctx->buflen += add;
  ...
  if (len > 0)
    {
      ...
      dMemcpy (&((char *) ctx->buffer)[left_over], buffer, len);
      left_over += len;
      if (left_over >= 64)
	{
	  sha1_process_block (ctx->buffer, 64, ctx);
	  left_over -= 64;
	   dMemcpy (ctx->buffer, &ctx->buffer[16], left_over);
	}
...
{
  const	U32 *words = static_cast<const U32 *>( buffer );
...
{
  char const *s = static_cast<const char*>(src);
	   ...
	   const void *in, size_t inlen, void *resbuf)
{
  ...
  dMemset (block, IPAD, sizeof (block));
  ...
  dMemset (block, OPAD, sizeof (block));
  memxor (block, key, keylen);
