...
bool Graph_SearchDFS<graph_type>::Search()
{
  ...
  while (!stack.empty())
  {
    ...
    //visited node)
    class graph_type::ConstEdgeIterator ConstEdgeItr(m_Graph, Next->To());
...
void Graph_SearchDijkstra<graph_type>::Search()
{
  ...
  while(!pq.empty())
  {
    ...
    class graph_type::ConstEdgeIterator ConstEdgeItr(m_Graph, NextClosestNode);
...
private:
  ...
  void Search(const int source)
  {
    ...
    while(!pq.empty())
    {
      ...
      class graph_type::ConstEdgeIterator ConstEdgeItr(m_Graph, best);
