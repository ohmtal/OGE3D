...
void WeightNavGraphNodeEdges(graph_type& graph, int node, double weight)
{
  ...
  class graph_type::ConstEdgeIterator ConstEdgeItr(graph, node);
...
std::vector<std::vector<int> > CreateAllPairsTable(const graph_type& G)
{
  ...
  for (int source=0; source<G.NumNodes(); ++source)
  {
    ...
    std::vector<const class graph_type::EdgeType*> spt = search.GetSPT();
...
std::vector<std::vector<F32> > CreateAllPairsCostsTable(const graph_type& G)
{
    //create a two dimensional vector
    std::vector<F32> row(G.NumNodes(), 0.0);
    std::vector<std::vector<F32> > PathCosts(G.NumNodes(), row);
    ...
    for (int source = 0; source < G.NumNodes(); ++source)
    {
        ...
        for (int target = 0; target < G.NumNodes(); ++target)
        {
            if (source != target)
            {
                PathCosts[source][target] = (F32)search.GetCostToNode(target);
            }
        }//next target node
...
double CalculateAverageGraphEdgeLength(const graph_type& G)
{
  ...
  class graph_type::ConstNodeIterator NodeItr(G);
  const class graph_type::NodeType* pN;
  for (pN = NodeItr.begin(); !NodeItr.end(); pN=NodeItr.next())
  {
    class graph_type::ConstEdgeIterator EdgeItr(G, pN->Index());
    for (const class graph_type::EdgeType* pE = EdgeItr.begin(); !EdgeItr.end(); pE=EdgeItr.next())
    {
...
{
  double greatest = GraphUtils::MinDouble;
