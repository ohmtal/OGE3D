...
struct Vector2D
{
  ...
  bool isZero()const{return (x*x + y*y) < GraphUtils::MinDouble;}
  ...
  {
    return (GraphUtils::isEqual(x, rhs.x) && GraphUtils::isEqual(y,rhs.y) );
  }
...
inline Vector2D POINTStoVector(const Point2F& p)
{
  return Vector2D(p.x, p.y);
}

inline Vector2D POINTtoVector(const Point2F& p)
{
  return Vector2D((double)p.x, (double)p.y);
}

inline Point2F VectorToPOINTS(const Vector2D& v)
{
   Point2F p;
  p.x = (short)v.x;
  p.y = (short)v.y;
...
inline Point2F VectorToPOINT(const Vector2D& v)
{
  Point2F p;
  p.x = (long)v.x;
  p.y = (long)v.y;
