...
/*
XXTH fix gcc :  error: need 'typename' before 'map::iterator' because 'map' is a dependent scope
  from https://stackoverflow.com/questions/610245/where-and-why-do-i-have-to-put-the-template-and-typename-keywords
 template <typename T>
...
{
	for (class container::iterator it = c.begin(); it != c.end(); ++it)
	{
		delete* it;
		*it = NULL;
	}
...
{
  for (class map::iterator it = m.begin(); it!=m.end(); ++it)
  {
    delete it->second;
    it->second = NULL;
  }
