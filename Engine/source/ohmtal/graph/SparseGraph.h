...
//
// Modified for tests with OGE
// 
//  $Grid = new Pathfinder();
//  $Grid.init(% this.rect.getAsInt(), $squareSize);
//  
...
public:
  ...
  typedef class std::vector<node_type>   NodeVector;
  typedef class std::list<edge_type>     EdgeList;
  typedef class std::vector<EdgeList>    EdgeListVector;
...
public:
  ...
  void RemoveEdges()
  {
    for (class EdgeListVector::iterator it = m_Edges.begin(); it != m_Edges.end(); ++it)
    {
      it->clear();
    }
  }
...
bool SparseGraph<node_type, edge_type>::isEdgePresent(int from, int to)const
{
    ...
    {
       for (class EdgeList::const_iterator curEdge = m_Edges[from].begin();
...
edge_type& SparseGraph<node_type, edge_type>::GetEdge(int from, int to)
{
  ...
  for (class EdgeList::iterator curEdge = m_Edges[from].begin();
       curEdge != m_Edges[from].end();
       ++curEdge)
  {
    if (curEdge->To() == to) return *curEdge;
...
void SparseGraph<node_type, edge_type>::RemoveEdge(int from, int to)
{
  ...
  class EdgeList::iterator curEdge;
...
bool SparseGraph<node_type, edge_type>::Save(std::ofstream& stream)const
{
  ...
  class NodeVector::const_iterator curNode = m_Nodes.begin();
  ...
  {
    for (class EdgeList::const_iterator curEdge = m_Edges[nodeIdx].begin();
