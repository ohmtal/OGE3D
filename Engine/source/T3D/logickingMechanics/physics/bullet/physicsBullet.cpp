...
#include "T3D/logickingMechanics/physics/bullet/physShapeSoftBullet.h"
#include "T3D/logickingMechanics/physics/bullet/physJointBullet.h"
...
// Callback contact
bool PhysicsBullet::contactAddedCallback(
	btManifoldPoint& cp,
	...
	int index1)
{
	if(cp.getLifeTime() == 0)
	{
      btVector3 vel = colObj0Wrap->getCollisionObject()->getInterpolationLinearVelocity()
         - colObj1Wrap->getCollisionObject()->getInterpolationLinearVelocity();
		...
		{
         /* 
			PhysBody* owner0 = dynamic_cast<PhysBody*>( PhysicsUserData::getObject(colObj0->getUserPointer()));
			PhysBody* owner1 = dynamic_cast<PhysBody*>( PhysicsUserData::getObject(colObj1->getUserPointer()));
         */
         PhysBody* owner0 = dynamic_cast<PhysBody*>(PhysicsUserData::getObject(colObj0Wrap->getCollisionObject()->getUserPointer()));
         PhysBody* owner1 = dynamic_cast<PhysBody*>(PhysicsUserData::getObject(colObj1Wrap->getCollisionObject()->getUserPointer()));
         if(owner0)
