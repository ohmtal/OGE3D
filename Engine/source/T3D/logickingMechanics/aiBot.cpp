...
#include "T3D/logickingMechanics/aiBot.h"
...
// Console Functions
// --------------------------------------------------------------------------------------------
DefineEngineStringlyVariadicMethod( AIBot, getEnemy, S32, 2, 2, "()")
{
	return object->getEnemyId();
}

DefineEngineStringlyVariadicMethod( AIBot, setEnemy, void, 3, 3, "(GameBase obj)")
{
	// Find the target
...
DefineEngineStringlyVariadicMethod( AIBot, hasEnemy, bool, 2, 2, "()")
{
	return object->getEnemy() ? true : false;
}

DefineEngineStringlyVariadicMethod( AIBot, distToEnemy, F32, 2, 2, "()")
{
	return object->getDistToEnemy();
}

DefineEngineStringlyVariadicMethod( AIBot, enableChase, void, 5, 5, "()")
{
	return object->enableChase(dAtob(argv[2]), dAtof(argv[3]), dAtof(argv[4]));
}

DefineEngineStringlyVariadicMethod( AIBot, enableFlee, void, 5, 5, "()")
{
	return object->enableFlee(dAtob(argv[2]), dAtof(argv[3]), dAtof(argv[4]));
}

DefineEngineStringlyVariadicMethod( AIBot, enableStrafe, void, 6, 6, "()")
{
	return object->enableStrafe(dAtob(argv[2]), dAtof(argv[3]), dAtof(argv[4]), dAtoi(argv[5]));
}


DefineEngineStringlyVariadicMethod( AIBot, attackParams, void, 4, 4, "()")
{
	return object->attackParams(dAtof(argv[2]), dAtof(argv[3]));
}


DefineEngineStringlyVariadicMethod( AIBot, allowMovement, void, 3, 3, "()")
{
	return object->allowMovement(dAtob(argv[2]));
}

DefineEngineStringlyVariadicMethod( AIBot, allowTacticalMovement, void, 3, 3, "()")
{
	return object->allowTacticalMovement(dAtob(argv[2]));
}


DefineEngineStringlyVariadicMethod( AIBot, isDead, bool, 2, 2, "()")
{
